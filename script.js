...
const citiesApi = 'http://api.travelpayouts.com/data/ru/cities.json',
    ...
    MAX_COUNT = 10; //количество карточек
...
const renederCheapDay = (cheapTicket) => {
...
const renederCheapYear = (cheapTickets) => {
...
const renederCheap = (data, date) => {
    const cheapTicketYear = JSON.parse(data).best_prices;
    const cheapTicketDay = cheapTicketYear.filter((item) => {
        return item.depart_date === date;
    })
 
    renederCheapDay(cheapTicketDay)
    renederCheapYear(cheapTicketYear);
...
formSearch.addEventListener('submit', (event) => {
    ...
    if (formData.from && formData.to) {
        ...
        getData(calendar + requestData, 
            (data) => {
            renederCheap(data, formData.when);
            }, 
            (error) => {
